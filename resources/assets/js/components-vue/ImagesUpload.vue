...
export default {
    ...
    methods: {
        ...
        updateFilesOrder: function () {
            ...
            var draggableItems = document.querySelectorAll(
                '#filegroup-' + this.settings.group + '-' + this.settings.locale + ' .draggable-item'
            );
            ...
            for (var i = 0; i < draggableItems.length; i++) {
                ...
                if (!itemId) {
                    var input = draggableItems[i].querySelector(
                        'input[name="files[' +
                            this.settings.group.replace('files-', '') +
                            '][' +
        ...
        handleSortingEnter: function (e) {
            ...
            var target = e.target.classList.contains('draggable-item')
                    ? e.target
                    : this.findAncestor(e.target, '.draggable-item'),
                draggedItemId = this.sortSource.dataset.itemId,
                targetItemId = target.dataset.itemId,
                oldIndex = this.items.findIndex(function (item) {
                    return item.id == draggedItemId;
                }),
                newIndex = this.items.findIndex(function (item) {
