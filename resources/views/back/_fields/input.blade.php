@if($field->getAppend() && $field->getPrepend())
    <div class="input-addon">
        ...
        <input type="text" name="{{ $name ?? $key }}" id="{{ $key }}" class="input inset-s" placeholder="{{ $field->getPlaceholder($locale ?? null) }}" value="{{ old($key, $manager->fieldValue($field, $locale ?? null)) }}">
