@if($field->getPrepend())
    <div class="input-addon">
        ...
        <input max="3000-01-01" type="date" name="{{ $name ?? $key }}" id="{{ $key }}" class="input inset-s" placeholder="{{ $field->getPlaceholder($locale ?? null) }}" value="{{ old($key, optional($field->getValue($locale ?? null))->format('Y-m-d')) }}">
    </div>
