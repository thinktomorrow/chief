@if($field->prepend)
    <div class="input-addon">
        ...
        <input type="date" name="{{ $name ?? $key }}" id="{{ $key }}" class="input inset-s" placeholder="{{ $field->placeholder ?? '' }}" value="{{ old($key, optional($manager->fieldValue($field, $locale ?? null))->format('Y-m-d')) }}">
    </div>
@elseif($field->append)
    <div class="input-addon">
        <input type="date" name="{{ $name ?? $key }}" id="{{ $key }}" class="input inset-s" placeholder="{{ $field->placeholder ?? '' }}" value="{{ old($key, optional($manager->fieldValue($field, $locale ?? null))->format('Y-m-d')) }}">
