...
<style>
.chief_widget{
    position: fixed; right: 0; left: 0; bottom: 1rem; z-index: 9999;
    width: 250px;margin:0 auto;
    background-color: white; color: #533D4D; border: 1px solid #FCE4DD; border-radius: 9999px;
...
.boundary:not(:last-child){ border-right: 1px solid #FCE4DD; }
.cloaked{ display: none;}
</style>
<!-- chief admin toast -->
<div id="chiefAdminToast" class="chief_widget">
    @if($editUrl)
    ...
    @endif
    <a class="inset-block boundary" href="{{ $previewModeToggleUrl }}" title="{{ $inPreviewMode ? 'Schakel preview uit: verberg offline pagina\'s' : 'Schakel preview aan: toon offline pagina\'s' }}">Preview {{ $inPreviewMode ? 'aan' : 'uit' }}</a>
