...
(function () {
    ...
    Dom.prototype = {
        ...
        nextElement: function nextElement(selector) {
            return this._getSibling(selector, 'nextElementSibling');
        },
        prev: function prev(selector) {
            return this._getSibling(selector, 'previousSibling');
        },
        prevElement: function prevElement(selector) {
        ...
        attr: function attr(name, value, data) {
            ...
            return this.each(function (node) {
                ...
                for (var key in obj) {
                    if (node.nodeType !== 3) {
                        if (key === 'checked') node.checked = obj[key];else node.setAttribute(data + key, obj[key]);
                    }
                }
            });
        },
            ...
            if (name === undefined) {
                ...
                for (var key in attrs) {
                    if (reDataAttr.test(attrs[key].nodeName)) {
                        ...
                        if (this._isObjectString(val)) val = this._toObject(val);else val = this._isNumber(val) ? parseFloat(val) : this._getBooleanFromStr(val);
        ...
        _getSibling: function _getSibling(selector, method) {
        ...
        _toObject: function _toObject(str) {
        ...
        _isObjectString: function _isObjectString(str) {
            return str.search(/^{/) !== -1;
        },
    ...
    App.prototype = {
        ...
        // draginside
        isDragInside: function isDragInside() {
            return this.dragInside;
        },
        setDragInside: function setDragInside(dragInside) {
            this.dragInside = dragInside;
        },
        isDragComponentInside: function isDragComponentInside() {
            return this.dragComponentInside;
        },
        setDragComponentInside: function setDragComponentInside(dragInside) {
            this.dragComponentInside = dragInside;
        },
        getDragComponentInside: function getDragComponentInside() {
            return this.dragComponentInside;
        },
    ...
    $R.add('class', 'toolbar.dropdown', {
        ...
        getItemsByClass: function getItemsByClass(classname) {
            ...
            for (var key in this.$items) {
                if (_typeof(this.$items[key]) === 'object' && this.$items[key].hasClass(classname)) {
    ...
    $R.add('service', 'cleaner', {
        ...
        input: function input(html, paragraphize, started) {
            // pre/code
    ...
    $R.add('class', 'cleaner.paragraphize', {
        ...
        // private
        _convert: function _convert(html) {
            // build markup tag
        ...
        _storeTags: function _storeTags(html) {
            var self = this;
            var $wrapper = this.utils.buildWrapper(html);
        ...
        _restoreTags: function _restoreTags(html) {
        ...
        _trimEmptyLines: function _trimEmptyLines(html) {
            var str = '';
            var arr = html.split("\n");
        ...
        _isConverted: function _isConverted(html) {
            if (this._isDisabled(html)) return html;else if (this._isEmptyHtml(html)) return this.opts.emptyHtml;else return true;
        },
    ...
    $R.add('service', 'inline', {
        ...
        _convertTags: function _convertTags(tag) {
            if (this.tag !== tag) {
                var $editor = this.editor.getElement();
        ...
        _revertTags: function _revertTags(tag) {
            var $editor = this.editor.getElement();
        ...
        _convertToStrike: function _convertToStrike(inlines) {
        ...
        _revertToInlines: function _revertToInlines() {
            var nodes = [];
            var $editor = this.editor.getElement();
        ...
        _normalizeBlocks: function _normalizeBlocks(nodes) {
            var tags = this.opts.inlineTags;
        ...
        _clearDecoration: function _clearDecoration() {
            var $editor = this.editor.getElement();
        ...
        _forceRemoveClass: function _forceRemoveClass($node, classname) {
    ...
    $R.add('service', 'utils', {
        ...
        removeInvisibleChars: function removeInvisibleChars(html) {
            return html.replace(/\uFEFF/g, '');
        },
        trimInvisibleChars: function trimInvisibleChars(direction) {
        ...
        // wrapper
        buildWrapper: function buildWrapper(html) {
            return $R.dom('<div>').html(html);
        },
        getWrapperHtml: function getWrapperHtml($wrapper) {
        ...
        getElementsFromHtml: function getElementsFromHtml(html, selector, exclude) {
            ...
            // array map polyfill
            var mapping = function mapping(callback, thisArg) {
    ...
    $R.add('class', 'editor.events', {
        ...
        blur: function blur(e) {
            ...
            var targets = ['.redactor-in-' + this.uuid, '.redactor-toolbar', '.redactor-dropdown', '.redactor-context-toolbar', '#redactor-modal-box', '#redactor-image-resizer'];
    ...
    $R.add('module', 'modal', {
        ...
        _close: function _close(e) {
            ...
            if (this.selectionMarkers) this.selection.restoreMarkers();else this.selection.restore();
    ...
    $R.add('class', 'input.arrow', {
        ...
        _init: function _init(e) {
            ...
            if (this.key === this.keycodes.LEFT) this.utils.trimInvisibleChars('left');else if (this.key === this.keycodes.RIGHT) this.utils.trimInvisibleChars('right');
        },
        ...
        _exitNextElement: function _exitNextElement(e, node) {
            ...
            if (node.nextElementSibling) this.caret.setStart(node.nextElementSibling);else this.utils.createMarkup(node);
    ...
    $R.add('class', 'input.enter', {
        ...
        _insertBreak: function _insertBreak(e) {
            ...
            if (data.isComponent() && !data.isComponentEditable() || data.isCode()) return;else if (data.isPre()) this.insertion.insertNewline();else this.insertion.insertBreakLine();
        },
    ...
    $R.add('module', 'upload', {
        ...
        _buildInput: function _buildInput() {
            ...
            if (this.opts.multipleUpload) this.$el.attr('multiple', 'multiple');else this.$el.removeAttr('multiple');
    ...
    $R.add('class', 'image.component', {
        ...
        _set_align: function _set_align(align) {
            ...
            if (_typeof(this.opts.imagePosition) === 'object') {
...
module.exports = __webpack_require__("./resources/assets/js/native.js");
