...
class AuditModelTest extends TestCase
{
    ...
    public function it_logs_create_events_on_pages()
    {
        ...
        $response = $this->actingAs($user, 'chief')
            ->post(route('chief.back.managers.store', Single::registrationKey()), $this->validPageParams());
    ...
    public function it_logs_edit_events_on_pages()
    {
        ...
        $response = $this->actingAs($user, 'chief')
            ->put(route('chief.back.managers.update', [Single::registrationKey(), $page->id]), $this->validUpdatePageParams());
    ...
    public function it_logs_delete_events_on_pages()
    {
        ...
        $this->actingAs($user, 'chief')
            ->post(route('chief.back.managers.store', Single::registrationKey()), $this->validPageParams(['published' => false]));

        $page = Page::first();

        $response = $this->actingAs($user, 'chief')
             ->delete(route('chief.back.managers.delete', [Single::registrationKey(), $page->id]), ['deleteconfirmation' => 'DELETE']);
    ...
    public function it_logs_archive_events_on_pages()
    {
        ...
        $this->actingAs($user, 'chief')
             ->post(route('chief.back.assistants.archive', [Single::registrationKey(), $page->id]));
