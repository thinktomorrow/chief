<div x-data="tableFilters" class="flex items-start gap-2">
    @foreach ($this->getSecondaryFilters() as $filter)
        <div data-filter-key="{{ $filter->getKey() }}">{!! $filter->render() !!}</div>
    @endforeach

    <div>
        @if (count($this->getTertiaryFilters()) > 0)
            <x-chief::button
                x-on:click="$dispatch('open-dialog', { id: 'table-filters-drawer-{{ $this->getId() }}' })"
                ...
                @if (($tertiaryFilterCount = $this->getTertiaryFilterCount()) > 0)
                    <div class="absolute -right-1.5 -bottom-1.5">
                        <div class="bg-primary-500 flex size-5 items-center justify-center rounded-full">
                            <span class="text-xs font-medium text-white"> {{ $tertiaryFilterCount }} </span>
                        </div>
                    </div>
                @endif
            </x-chief::button>
        ...
        <template x-teleport="body">
            <x-chief::dialog.drawer id="table-filters-drawer-{{ $this->getId() }}" title="Meer filters">
                <div class="space-y-6">
                    ...
                    @foreach ($this->getTertiaryFilters() as $filter)
                        <div class="space-y-2">
                            <div>
                                @if ($filter->getLabel())
                                    <x-chief::form.label> {{ $filter->getLabel() }} </x-chief::form.label>
                                @endif
