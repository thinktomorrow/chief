<div id="table-container-header-sorters" class="flex items-start justify-end gap-2">
    @if (count($this->getActiveFilters()) > 0)
        <div class="flex items-center justify-end gap-2">
            <span class="text-grey-500 text-sm leading-5">{{ $this->resultTotal }} items</span>
    ...
    @if (count($this->getSortersForView()) > 1)
        <x-chief::button
            x-on:click="$dispatch('open-dialog', { 'id': 'table-sorting-{{ $this->getId() }}' })"
            ...
            @if (count($this->getActiveSorters()) > 0)
                @foreach ($this->getActiveSorters() as $sorter)
                    @if (! $sorter->hiddenFromView() && $sorter->showsActiveLabel())
                        <span> {{ $sorter->getLabel() }} </span>
                    @endif
                @endforeach
            @endif
        </x-chief::button>

        <x-chief::dialog.dropdown id="table-sorting-{{ $this->getId() }}" placement="bottom-end">
            <div
                x-on:change="
                    close();
                    $wire.addSorter();
