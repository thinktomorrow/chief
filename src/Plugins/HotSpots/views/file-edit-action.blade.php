<div>
    @if($previewFile->isImage())
        <button type="button" wire:click="openImageHotSpots()" title="Image hotspot tool" class="shrink-0">
            <x-chief::button>
                ...
                <span class="inline-flex gap-1.5">
                    ...
                    @if(is_array($previewFile->getData('hotspots')) && $hotSpotCount = count($previewFile->getData('hotspots')))
                        <span class="flex items-center justify-center w-5 h-5 text-xs font-medium rounded-full bg-black/10">
                            {{ $hotSpotCount }}
                        </span>
                    @endif
                </span>
