...
<x-chief::window title="{{ $this->allowMultipleItems() || count($items) > 1 ? 'Menu' : '' }}">
        ...
        @foreach ($items as $item)
            <div wire:key="menu-tab-content-{{ $item->id }}">
                @if ($item->id === $activeItemId)
                    ...
                    <livewire:chief-wire-table::data-table
    ...
    <template x-teleport="body">
        <livewire:chief-wire-menu::add-menu :type="$type" :locales="$locales" />
    </template>

    <template x-teleport="body">
        <livewire:chief-wire-menu::edit-menu :type="$type" :locales="$locales" />
    </template>
</x-chief::window>
