...
trait WithUniqueSlug
{
    ...
    private function cleanupIdenticalSlugs(Visitable $model, string $siteId, string $slug, ?int $whiteListedId = null): void
    {
        ...
        $this->deleteIdenticalRedirects($siteId, $slug);

        if ($this->force) {
            $this->deleteIdenticalRecords($model, $siteId, $slug, $whiteListedId);
            $this->deleteOtherIdenticalRecords($model, $siteId, $slug);
        }
    }
     ...
     */
    private function deleteIdenticalRedirects(string $siteId, string $slug): void
    {
        $sameUrls = UrlRecord::where('slug', $slug)->where('site', $siteId)->get();

        $sameUrls->filter(function ($url) use ($siteId) {
            return $url->site == $siteId && $url->isRedirect();
     ...
     */
    private function deleteIdenticalRecords(Visitable $model, string $siteId, string $slug, ?int $whiteListedId): void
    {
     ...
     */
    private function deleteOtherIdenticalRecords(Visitable $model, string $siteId, string $slug): void
    {
        $sameUrls = UrlRecord::where('slug', $slug)
            ->where('site', $siteId)
    ...
    private function assertSlugDoesNotExistsAsActiveUrl(string $site, string $slug, ?int $whiteListedId = null)
    {
        ...
        if ($existingRecord && $existingRecord->id != $whiteListedId) {
            throw new \Exception('An Url record (other than id: '.$whiteListedId.') already exists for site: '.$site.', slug: '.$slug);
        }
    }
}
