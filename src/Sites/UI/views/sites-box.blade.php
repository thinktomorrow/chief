...
<div class="flex justify-end gap-2">
    ...
    <x-chief::dialog wired title="Actief op deze sites" size="xs">
                ...
                <x-chief::input.group class="w-64">
                    <div class="flex items-start gap-2 ">
                        <x-chief::input.checkbox
                                id="{{ $site->id }}"
                                :checked="in_array($site->id, $activeSites)"
                                wire:model.live="activeSites"
                                value="{{ $site->id }}"
                        ></x-chief::input.checkbox>
                        <x-chief::input.label for="{{ $site->id }}">{{ $site->name }}</x-chief::input.label>
                    </div>
                </x-chief::input.group>
            @endforeach
