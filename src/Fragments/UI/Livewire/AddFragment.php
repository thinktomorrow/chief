...
class AddFragment extends Component
{
    ...
    public function attachFragment(string $fragmentId, ?string $sourceContextId = null)
    {
        ...
        if ($this->parentId) {
            $this->dispatch('fragment-added-'.$this->parentComponentId, ...$eventPayload)->to('chief-wire-fragments::edit-fragment');
        } else {
            $this->dispatch('fragment-added-'.$this->parentComponentId, ...$eventPayload)->to('chief-wire-fragments::fragment-context');
        }
