...
<x-chief::window title="Fragmenten">
        ...
        @foreach ($items as $item)
            <div wire:key="context-tab-content-{{ $item->id }}">
                @if ($item->id === $activeItemId)
                    ...
                    <livewire:chief-wire-fragments::fragment-context
                        :key="$item->id"
                        :model="$model"
                        :context="$item"
    ...
    <template x-teleport="body">
        <livewire:chief-wire-fragments::add-context :model-reference="$model->modelReference()" :locales="$locales" />
    </template>
    <template x-teleport="body">
        <livewire:chief-wire-fragments::edit-context :model-reference="$model->modelReference()" :locales="$locales" />
    </template>
</x-chief::window>
