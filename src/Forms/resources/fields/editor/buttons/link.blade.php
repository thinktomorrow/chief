...
<x-chief::dialog.dropdown id="tiptap-header-link-dropdown-{{ $locale }}" placement="bottom-center">
    ...
    >
        <x-chief::input.group>
            <x-chief::input.label class="text-sm">Tekst</x-chief::input.label>
            <x-chief::input.text x-model="text" />
        </x-chief::input.group>
        ...
        <x-chief::input.group>
            <div class="flex justify-between gap-2">
                ...
                <x-chief::dialog.drawer id="tiptap-header-link-modal-{{ $locale }}" title="Kies een pagina">
                    <x-chief::input.group>
                        {{-- <x-chief::input.label class="text-sm">Kies een pagina</x-chief::input.label> --}}
                        <x-chief::multiselect
                            x-model="page"
        ...
        <x-chief::input.group inner-class="flex items-start gap-2">
            ...
            <x-chief::input.label for="is-target-blank" unset class="body body-dark text-sm leading-5">
                Open in nieuw tablad
            </x-chief::input.label>
        </x-chief::input.group>

        <x-chief::input.group>
            <x-chief::input.label class="text-sm">Layout</x-chief::input.label>
            <x-chief::multiselect
                x-model="classList"
                :options="Thinktomorrow\Chief\Forms\Fields\Concerns\Select\PairOptions::convertOptionsToChoices(
                    Thinktomorrow\Chief\Forms\Fields\Concerns\Select\PairOptions::toPairs([
