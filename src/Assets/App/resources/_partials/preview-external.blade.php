<div class="shrink-0">
    ...
    @else
        <div class="flex items-center justify-center w-16 h-16 rounded-lg bg-grey-100">
            <svg width="24" height="24" class="text-grey-400"><use xlink:href="#icon-document"/></svg>
        </div>
    @endif
</div>
...
<div class="flex items-center py-2 grow">
    <div class="space-y-0.5 leading-tight">
        ...
        <p class="text-sm text-grey-500">
            <span class="text-xs label label-info">{{ $file->getExternalAssetType() }}</span>
...
<div class="flex items-center gap-2 py-2 pr-2 shrink-0">
    ...
    @elseif($file->isUploading && isset($this->findUploadFile($file->id)['progress']) && $this->findUploadFile($file->id)['progress'] <= 100)
        ...
        <div x-data="{progress: @entangle('files.'.$this->findUploadFileIndex($file->id).'.progress')}" class="w-full h-2 rounded bg-grey-50">
        ...
        @if($file->isQueuedForDeletion)
            @if($file->mediaId)
                <span class="text-sm text-primary-500">
                    Wordt verwijderd na bewaren.
                    <span
                        class="underline cursor-pointer link link-primary"
                        wire:click="undoDeleteFile('{{ $file->id }}')"
                    >
                        Ongedaan maken
                    </span>
                </span>
