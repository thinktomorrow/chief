<div wire:sortable.item="{{ $file->id }}" class="relative @container">
    ...
    <div class="relative flex justify-between gap-x-4 gap-y-2 py-2 pl-2 pr-4 @1px:flex-wrap @md:flex-nowrap">
        ...
        @if(!$file->isValidated)
            <div class="absolute inset-0 flex items-center justify-between gap-2 px-4 py-2 backdrop-blur-sm backdrop-filter bg-red-100/75 @1px:flex-wrap @md:flex-nowrap">
        ...
        @elseif($file->isQueuedForDeletion)
            <div class="absolute inset-0 flex items-center justify-between gap-2 px-4 py-2 backdrop-blur-sm backdrop-filter bg-white/75 @1px:flex-wrap @md:flex-nowrap">
                @if($file->mediaId)
